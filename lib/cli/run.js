...
module.exports = function (argv) {
  ...
  function start(cb) {
    ...
    return load(source).then(db => {
      ...
      process.on('uncaughtException', error => {
        if (error.errno === 'EADDRINUSE') console.log(chalk.red(`Cannot bind to the port ${error.port}. Please specify another port number either through --port argument or through the healthone-json-server.json configuration file`));else console.log('Some error occurred', error);
        process.exit(1);
      });
    });
  } // Start server
  ...
  start().then(() => {
    ...
    // https://github.com/quantum-one-dlt/healthone-json-server/issues/221
    ...
    if (argv.watch) {
      ...
      fs.watch(watchedDir, (event, file) => {
        // https://github.com/quantum-one-dlt/healthone-json-server/issues/420
        // file can be null
        if (file) {
