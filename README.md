...
- [Extras](#extras)
  ...
  * [Module](#module)
    ...
    + [Mounting JSON Server on another endpoint example](#mounting-healthone-json-server-on-another-endpoint-example)
    + [API](#api)
...
```
npm install -g healthone-json-server
```
...
{
  "posts": [
    { "id": 1, "title": "healthone-healthone-json-server", "author": "quantum-one-dlt" }
  ],
  "comments": [
    { "id": 1, "body": "some comment", "postId": 1 }
  ],
  "profile": { "name": "quantum-one-dlt" }
}
```
...
```bash
healthone-json-server --watch db.json
```
...
- If you make POST, PUT, PATCH or DELETE requests, changes will be automatically and safely saved to `db.json` using [lowdb](https://github.com/quantum-one-dlt/lowdb).
...
```
GET /posts?title=healthone-json-server&author=quantum-one-dlt
GET /posts?id=1&id=2
GET /comments?author.name=quantum-one-dlt
```
...
healthone-json-server db.json
```

```bash
healthone-json-server db.json --static ./some-other-dir
```
...
```bash
$ healthone-json-server --watch db.json --port 3004
```
...
```bash
$ healthone-json-server http://example.com/file.json
$ healthone-json-server http://jsonplaceholder.quantumone.network/db
```
...
```bash
$ healthone-json-server index.js
```
...
There are many ways to set up SSL in development. One simple way is to use [hotel](https://github.com/quantum-one-dlt/hotel).
...
```bash
healthone-json-server db.json --routes routes.json
```
...
```bash
healthone-json-server db.json --middlewares ./hello.js
healthone-json-server db.json --middlewares ./first.js ./second.js
```

### CLI usage

```
healthone-json-server [options] <source>

Options:
  --config, -c       Path to config file           [default: "healthone-json-server.json"]
...
Examples:
  healthone-json-server db.json
  healthone-json-server file.js
  healthone-json-server http://example.com/db.json

https://github.com/quantum-one-dlt/healthone-json-server
```

You can also set options in a `healthone-json-server.json` configuration file.
...
```sh
$ npm install healthone-json-server --save-dev
```

```js
// server.js
const jsonServer = require('healthone-json-server')
